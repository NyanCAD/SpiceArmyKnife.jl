...
#==============================================================================#
# Inlined Dual Decomposition Helpers
#
# These functions use method dispatch instead of runtime `isa` checks to
# extract values and partials from potentially nested duals. This allows
# the compiler to inline the correct method based on known types at compile
# time, eliminating dead branches and avoiding dynamic dispatch overhead.
#
# The key insight is that Julia's multiple dispatch is resolved at compile
# time when types are known, whereas `isa` checks are runtime operations
# that can prevent inlining and optimization.
#==============================================================================#
...
Returns a tuple suitable for Jacobian extraction and stamping.

# Returns
- `I_val::Float64`: Scalar current value
- `I_resist`: The resistive dual (for extracting ∂I/∂V partials), or scalar if no Jacobian
- `q_val::Float64`: Scalar charge value (0.0 if no reactive component)
- `I_react`: The reactive dual (for extracting ∂q/∂V partials), or scalar 0.0 if no reactive
- `has_reactive::Bool`: Whether the contribution contains ddt() terms

This replaces runtime `isa` checks like:
```julia
if I_branch isa ForwardDiff.Dual{ContributionTag}
    I_resist = value(I_branch)
    ...
...
```
"""
@inline function decompose_branch(x::Float64)
    # Scalar - no dual at all (constant contribution)
    (x, x, 0.0, 0.0, false)
end

@inline function decompose_branch(x::T) where {T<:Real}
    # Other real types (Int, etc.) - convert to Float64
    v = Float64(x)
    (v, v, 0.0, 0.0, false)
end
...
@inline function decompose_branch(x::Dual{ContributionTag,Dual{JacobianTag,T,N},1}) where {T,N}
    # Has ddt: ContributionTag wrapping JacobianTag dual
...
"""
    decompose_voltage_contrib(x) -> (V_val, V_resist, V_react, has_reactive)

Decompose a voltage contribution value into its components.
Similar to decompose_branch but for voltage contributions V(p,n) <+ expr.

# Returns
- `V_val::Float64`: Scalar voltage value (resistive part)
- `V_resist`: The resistive component (for value extraction)
- `V_react::Float64`: Reactive coefficient (e.g., L from L*ddt(I))
- `has_reactive::Bool`: Whether the contribution contains ddt() terms
"""
@inline function decompose_voltage_contrib(x::Float64)
    (x, x, 0.0, false)
end

@inline function decompose_voltage_contrib(x::T) where {T<:Real}
    v = Float64(x)
    (v, v, 0.0, false)
end
...
"""
    get_partial(x, ::Val{K}) -> Float64

Extract the K-th partial from a dual number, returning 0.0 for non-duals.
Uses method dispatch instead of runtime `isa` checks.

This replaces patterns like:
```julia
dI_dVk = I_resist isa ForwardDiff.Dual ? ForwardDiff.partials(I_resist, k) : 0.0
```
"""
