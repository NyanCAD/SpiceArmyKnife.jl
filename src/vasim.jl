...
# Note: ContributionStatement Scope method removed - DAECompiler-specific
# MNA uses MNAScope for contributions (see below)
...
# Note: make_spice_device function removed - it was DAECompiler-specific
# Use make_mna_device instead (see below)
...
Generates `stamp!` methods that work with MNAContext directly for
circuit simulation using the Modified Nodal Analysis approach.
