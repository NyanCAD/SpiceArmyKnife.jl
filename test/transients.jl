...
@testset "PWL" begin
    ...
    * PWL test
    ...
    function PWLIRcircuit(params, spec; x=Float64[])
        ...
        stamp!(PWLCurrentSource(times, values; name=:I), ctx, vout, 0; t=spec.time, mode=spec.mode)
...
@testset "Butterworth Filter" begin
    ...
    *Third order low pass filter, butterworth, with ω_c = 1
    ...
    function butterworth_circuit(params, spec; x=Float64[])
        ...
        stamp!(SinVoltageSource(0.0, 1.0, ω_val/2π; name=:V), ctx, vin, 0; t=spec.time, mode=spec.mode)
